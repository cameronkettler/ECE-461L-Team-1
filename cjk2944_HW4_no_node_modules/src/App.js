...
function ProjectRow({project}){
  // Load in all of our attributes passed in through project prop
  const { name, listAuthorizedUsers, HWSet1, HWSet2, alreadyJoined } = project;
  const [joined, setJoined] = useState(alreadyJoined)

  function handleInputClick(event) {
    // Clear the field when we click on the field box
    event.target.value = "";
  }

  function handleInputRevert(event) {
    // If nothing is in the field box, default it back to "Enter Quantity"
  ...
  function handleCheckIn(event) {
    // Get data from parent element (Text Entry Box)
    const parent = event.target.parentElement;
    // Receive data as an input, specify that it is a text input
    const input = parent.querySelector("input[type='text']");
    const value = parseInt(input.value);
    // If the value is not "NaN", we will update the counts
    if (!isNaN(value)) {
      // Specify the class
      ...
      if (hardwareElement) {
        ...
        if (newCount >= minCount) { 
          // Hard code to maximum of 100 hardware components
  ...
  function handleCheckOut(event) {
    ...
    if (!isNaN(value)) {
      ...
      if (hardwareElement) {
        ...
        // User is checking out more items, so add current value to the value passed by the parent
...
// Hard code STATE for now because we are not pulling from MongoDB yet
const STATE = [
